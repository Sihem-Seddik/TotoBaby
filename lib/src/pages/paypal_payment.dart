  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
          InAppWebView(
            initialUrlRequest: _con.url != null
                ? URLRequest(url: Uri.parse(_con.url))
                : URLRequest(
                    url: Uri.parse(
                        '${GlobalConfiguration().getString('base_url')}payments/paypal/express-checkout?api_token=${widget.routeArgument?.param ?? ''}&user_id=${widget.routeArgument?.id ?? ''}&delivery_address_id=${widget.routeArgument.param.id ?? ''}')),
